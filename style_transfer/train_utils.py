...
def train_step(dataset,style_activations,steps_per_epoch,transform_net,
	loss_model,optimizer,checkpoint_path="./",content_weight=1e4,
	style_weight=1e-2,total_variation_weight=0.004,content_layers_weights=[1],
...
def gatys_train_step(image_var,content_activations,style_activations,
    steps_per_epoch,loss_model,optimizer,content_weight=1e4,
    style_weight=1e-2,total_variation_weight=0.004,
    content_layers_weights=[1],style_layers_weights=[1]*5):
    batch_losses=[]
    for steps in range(steps_per_epoch):
        with tf.GradientTape() as tape:
            pred_activations=loss_model.get_activations(image_var)
            curr_loss=perceptual_loss(pred_activations,
                content_activations,style_activations,content_weight,
                style_weight,content_layers_weights,style_layers_weights)
...
def content_loss(placeholder,content,weight):
    assert (placeholder.shape == content.shape),f"dimension not same {placeholder.shape} not = {content.shape}"
...
def style_loss(placeholder,style,weight):
    assert (placeholder.shape == style.shape),f"dimension not same {placeholder.shape} not = {style.shape}"
    s=gram_matrix(style)
