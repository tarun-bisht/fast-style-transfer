from style_transfer.train_utils import (TensorflowDatasetLoader,train_step,
						get_loss_model)
...
parser.add_argument("--checkpoint","-ckpt",
					help="Path to save Trained Checkpoints")
parser.add_argument('--style_image',"-style",
					help='url or file path to style image')
parser.add_argument("--train_path","-tp",default='data/train',
					help="Dataset path where train folder with images is located")
parser.add_argument('--input_shape',"-shape",nargs='+',type=int,default=[256,256], 
					help='input image shape for training network')
parser.add_argument('--batch_size',"-batch",default=4,type=int,
					help='training batch size')
parser.add_argument('--learning_rate',"-lr",default=1e-3,type=float,
					help='number of training epochs')
parser.add_argument('--epochs',"-e",default=2,type=int,
					help='number of training epochs')
parser.add_argument('--content_layers_weights',"-clw",nargs='+',default=[1],type=float,
					help='Content weight with respect to each content layer respectively')
parser.add_argument('--style_layers_weights',"-slw",nargs='+',default=[1,1,1,1,1],
					type=float,help='Style weight with respect to each style layer respectively')
parser.add_argument('--content_weight',"-cw",default=1e1,type=float,
					help='Content weight')
parser.add_argument('--style_weight',"-sw",default=1e4,type=float,
					help='Style weight')
parser.add_argument('--total_variation_weight',"-tvw",default=0.004,type=float,
					help='Total variation weight')
parser.add_argument('--content_layers',"-cl",nargs='+',default=['block4_conv2'],
					help='Content layers to calculate loss on')
parser.add_argument('--style_layers',"-sl",nargs='+',
...
print(f"Dataset loaded! Number of images: ",len(loader))
print(f"Dataset Specs: ",dataset.element_spec)
...
for epoch in range(1,epochs+1):
	print(f"epoch: {epoch}")
	batch_loss=train_step(dataset,style_activations,steps_per_epoch,
						style_model,loss_model,optimizer,model_save_path,
